...
struct JetWithInfo
{
    ...
    JetWithInfo(TStarJetVectorJet _orig, double _pt, double _eta, double _phi, double _y, double _neutral_fraction,
                double n_constituents, int _event_id, double _weight, double _multiplicity,
                bool _is_rejected, double _vz) : orig(_orig),
                                                 pt(_pt),
                                                 eta(_eta),
                                                 phi(_phi),
                                                 y(_y),
...
{
    OutputTreeEntry () {};
    OutputTreeEntry (JetWithInfo jet) :vz(jet.vz), n_constituents(jet.n_constituents), weight(jet.weight), multiplicity(jet.multiplicity),is_rejected(jet.is_rejected), pt(jet.pt), eta(jet.eta), phi(jet.phi), neutral_fraction(jet.neutral_fraction) {};
...
typedef pair<JetWithInfo, JetWithInfo>
    MatchedJetWithInfo;
...
int MatchGeantToPythia(TString mcTreeName, TString OutFile = "test.root")
{
    ...
    MatchedTree->Branch("mc_neutral_fraction", &mc_result.neutral_fraction, "mc_neutral_fraction/D");
    MatchedTree->Branch("mc_n_constituents", &mc_result.n_constituents, "mc_n_constituents/I");
    MatchedTree->Branch("mc_event_id", &mc_result.vz, "mc_event_id/D");
    MatchedTree->Branch("mc_weight", &mc_result.weight, "mc_weight/D");
    MatchedTree->Branch("mc_multiplicity", &mc_result.multiplicity, "mc_multiplicity/D");
    MatchedTree->Branch("mc_is_rejected", &mc_result.is_rejected, "mc_is_rejected/O");
    ...
    MatchedTree->Branch("reco_neutral_fraction", &reco_result.neutral_fraction, "reco_neutral_fraction/D");
    MatchedTree->Branch("reco_n_constituents", &reco_result.n_constituents, "reco_n_constituents/I");
    ...
    MatchedTree->Branch("reco_multiplicity", &reco_result.multiplicity, "reco_multiplicity/D");
    MatchedTree->Branch("reco_is_rejected", &reco_result.is_rejected, "reco_is_rejected/O");
    ...
    for (Long64_t mcEvi = 0; mcEvi < N; ++mcEvi) // event loop
    {
        ...
        for (int j = 0; j < mc.njets; ++j)
        {
            ...
            mcresult.push_back(JetWithInfo(*mc_jet, mc_jet->Pt(), mc_jet->Eta(), mc_jet->Phi(),mc_jet->Rapidity(), 
            mc.neutral_fraction[j], mc.n_constituents[j], mc.eventid, mc.weight, mc.mult, mc.is_rejected, mc.vz));
        } // end of mcjet loop
        ...
        if (recoEvent >= 0)
        {
            ...
            // if (reco_accepted_list.find(reco.event_sum_pt) != reco_accepted_list.end())
            //     continue;
            // else
            ...
            for (int j = 0; j < reco.njets; ++j)
            {
                ...
                recoresult.push_back(JetWithInfo(*reco_jet, reco_jet->Pt(), reco_jet->Eta(), reco_jet->Phi(), reco_jet->Rapidity(),
                 reco.neutral_fraction[j], reco.n_constituents[j], reco.eventid, reco.weight, reco.mult, reco.is_rejected, reco.vz));
            }
        } // end of recojet loop
        ...
        if (mcresult.size() > 0)
        {
            for (vector<JetWithInfo>::iterator mcit = mcresult.begin(); mcit != mcresult.end();)
            {
                bool isMatched = false;
                if (recoresult.size()>0)
                {
                    for (vector<JetWithInfo>::iterator recoit = recoresult.begin(); recoit != recoresult.end();)
                    {
                ...
                {
                    JetWithInfo miss(*dummyjet, -9, -9, -9, -9, -9,-9, mc.eventid, mc.weight, mc.mult, mc.is_rejected, mc.vz);
        ...
        {
            JetWithInfo fake(*dummyjet, -9, -9, -9, -9, -9,-9, reco.eventid, reco.weight, reco.mult, reco.is_rejected, reco.vz);
        ...
        for (auto matched_jet : MatchedResult)
        {
            ...
            if(reco_result.pt>0){ 
                ...
                Double_t dphi =  TVector2::Phi_mpi_pi(matched_jet.first.phi - matched_jet.second.phi);
