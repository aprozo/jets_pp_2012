...
int ana_trees_simple(TString OutFile = "hists2.root") {
  ...
  TH1D *hist_sampled_lumi_VPDMB =
      (TH1D *)eventsFile->Get("hist_sampled_lumi_VPDMB");
  ...
  TH1D *hist_max_nevents_VPDMB =
      (TH1D *)eventsFile->Get("hist_max_nevents_VPDMB");
  TH1D *hist_sampled_lumi_JP2 =
      (TH1D *)eventsFile->Get("hist_sampled_lumi_JP2");
  ...
  TH1D *JP2 =
      new TH1D("JP2", "; p_{T} (GeV/c); Entries (per event)", 100, 0, 100);
  ...
  TH2D *JP2_run = new TH2D("JP2_run", "; p_{T} (GeV/c); Entries (per event)",
                           100, 0, 100, nRunBins, 0, nRunBins);
  ...
  for (int i = 0; i < jp2_tree->GetEntries(); ++i) {
    ...
    //   show progress
    if (i % 100000 == 0) {
      cout << "Processing JP2 entry: " << i << " / " << jp2_tree->GetEntries()
           << endl;
    }
    if (jp2.njets < 1)
    ...
    double weight_prescale = hist_prescale_VPDMB->GetBinContent(binRun) /
                             hist_prescale_JP2->GetBinContent(binRun);

    double weight_livetime = hist_livetime_JP2->GetBinContent(binRun) /
                             hist_livetime_VPDMB->GetBinContent(binRun);

    double weight_max_nevents = hist_max_nevents_VPDMB->GetBinContent(binRun) /
                                hist_max_nevents_JP2->GetBinContent(binRun);
  ...
  runRatio->cd();
  gPad->SetLogy();
  TH1D *weight_prescale_run =
      (TH1D *)hist_prescale_VPDMB->Clone("weight_prescale_run");
  ...
  TH1D *weight_livetime_run =
      (TH1D *)hist_livetime_JP2->Clone("weight_livetime_run");
  weight_livetime_run->Divide(hist_livetime_VPDMB);
  weight_livetime_run->SetLineColor(2001);
  weight_livetime_run->Draw("same");

  TH1D *weight_max_nevents_run =
      (TH1D *)hist_max_nevents_VPDMB->Clone("weight_max_nevents_run");
  ...
  TH1D *JP2_MaxNEvents_Ratio =
      (TH1D *)JP2_MaxNEvents->Clone("JP2_MaxNEvents_Ratio");
  JP2_MaxNEvents_Ratio->Divide(hPtMb);

  TH1D *JP2_TrigMatch_Ratio =
      (TH1D *)JP2_MatchTrigger->Clone("JP2_TrigMatch_Ratio");
