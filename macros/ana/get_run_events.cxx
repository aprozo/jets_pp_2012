...
std::map<int, RunInfo> CreateRunInfoMap(
    const char *filename = "../lists/files_lumi_jaime/lumi_VPDMB_JP2.csv") {
  // Open the file
  ...
  while (std::getline(file, line)) {
    try {
      ...
      std::getline(ss, token, ',');
      info.JP2_weight = std::stod(token);    // JP2_weight = (prescale_VPDMB/prescale_JP2)*(livetime_JP2/livetime_VPDMB)*(max_nevents_VPDMB/max_nevents_JP2)
...
int get_run_events() {
  ...
  std::map<int, RunInfo> runInfoMap =
      CreateRunInfoMap("../lists/files_lumi_james_dunlop/lumi_VPDMB_JP2.csv");
  ...
  TH1D *hist_sampled_lumi_VPDMB =
      (TH1D *)temp->Clone("hist_sampled_lumi_VPDMB");
  ...
  hist_JP2_weight->SetTitle("#frac{PS_{MB}}/{PS_{JP2}} #frac{livetime_{JP2}}/{livetime_{MB}} #frac{nevents_{MB}}/{nevents_{JP2}}; Run Number; (prescale_MB/prescale_JP2)*(livetime_JP2/livetime_MB)*(max_nevents_MB/max_nevents_JP2)");

  //   self-consistency check on max nevents
  TH1D *ratio_max_nevents_VPDMB =
      (TH1D *)hEventsRun_MB->Clone("ratio_max_nevents_VPDMB");
  ratio_max_nevents_VPDMB->Divide(hist_max_nevents_VPDMB);

  TH1D *ratio_max_nevents_JP2 =
      (TH1D *)hEventsRun_JP2->Clone("ratio_max_nevents_JP2");
