...
struct MyJet {
  ...
  MyJet():pt(-9), ptLead(-9), eta(-9), phi(-9), y(-9), neutral_fraction(-9),
          trigger_match_JP2(false), n_constituents(-9), event_id(-9), weight(-9),
          multiplicity(-9), trigger_match_HT2(false), vz(-9) {};

  MyJet(TStarJetVectorJet _orig, double _pt,double _ptLead, double _eta, double _phi,
...
int matching_mc_reco(TString mcTreeName="/gpfs01/star/pwg/prozorov/jets_pp_2012/output/mc/tree_pt-hat3545_033_R0.5.root") {
  ...
  if (mcTreeName.Contains("R0.2")) RCut = 0.2;
  else if (mcTreeName.Contains("R0.3")) RCut = 0.3;
  else if (mcTreeName.Contains("R0.4")) RCut = 0.4;
  else if (mcTreeName.Contains("R0.5")) RCut = 0.5;
  else if (mcTreeName.Contains("R0.6")) RCut = 0.6;
  else {
  ...
  TH1D *hMiss =
      new TH1D("hMiss", "Miss Rate; p_{t}, GeV/c", 500, 0, 50);
  TH1D *hFake =
      new TH1D("hFake", "Fake Rate; p_{t}, GeV/c", 500, 0, 50);
  ...
  MatchedTree->Branch("mc_neutral_fraction", &outMcJet.neutral_fraction,
                      "mc_neutral_fraction/D");
  MatchedTree->Branch("mc_trigger_match_JP2", &outMcJet.trigger_match_JP2,
                      "mc_trigger_match_JP2/O");

  MatchedTree->Branch("mc_n_constituents", &outMcJet.n_constituents,
                      "mc_n_constituents/I");
  // MatchedTree->Branch("mc_event_id", &outMcJet.vz, "mc_event_id/D");
  MatchedTree->Branch("mc_weight", &outMcJet.weight, "mc_weight/D");
  MatchedTree->Branch("mc_multiplicity", &outMcJet.multiplicity,
                      "mc_multiplicity/D");
  MatchedTree->Branch("mc_trigger_match_HT2", &outMcJet.trigger_match_HT2,
  ...
  MatchedTree->Branch("reco_neutral_fraction", &outRecoJet.neutral_fraction,
                      "reco_neutral_fraction/D");
  MatchedTree->Branch("reco_trigger_match_JP2", &outRecoJet.trigger_match_JP2,
                      "reco_trigger_match_JP2/O");
  MatchedTree->Branch("reco_n_constituents", &outRecoJet.n_constituents,
  ...
  MatchedTree->Branch("reco_multiplicity", &outRecoJet.multiplicity,
                      "reco_multiplicity/D");
  MatchedTree->Branch("reco_trigger_match_HT2", &outRecoJet.trigger_match_HT2,
  ...
  for (Long64_t iEvent = 0; iEvent < nEvents; ++iEvent) // event loop
  {
    ...
    for (int j = 0; j < mc.njets; ++j) {
      TStarJetVectorJet *tempMcJet =
          dynamic_cast<TStarJetVectorJet *>(mc.jets->At(j));

      mcJets.push_back(
          MyJet(*tempMcJet, tempMcJet->Pt(), mc.ptLead[j],
                      tempMcJet->Eta(), tempMcJet->Phi(),
                      tempMcJet->Rapidity(), mc.neutral_fraction[j],
    ...
    if (recoEvent >= 0) {
      ...
      for (int j = 0; j < reco.njets; ++j) {
        ...
        if (abs(tempRecoJet->Eta()) > EtaCut)
          continue;
        recoJets.push_back(MyJet(
            *tempRecoJet, tempRecoJet->Pt(),  reco.ptLead[j],
             tempRecoJet->Eta(), tempRecoJet->Phi(),
            tempRecoJet->Rapidity(), reco.neutral_fraction[j],
