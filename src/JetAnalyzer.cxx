...
JetAnalyzer::JetAnalyzer(std::vector<fastjet::PseudoJet> &InOrigParticles, fastjet::JetDefinition &JetDef,
   ...
   : fastjet::ClusterSequenceArea(InOrigParticles, JetDef, AreaDef),
     OrigParticles(InOrigParticles),
...
fastjet::Subtractor *JetAnalyzer::GetBackgroundSubtractor()
{
   ...
   // assert ( ymin == -ymax                            && "Can't construct ghosted area for asymmetric rapidity
   // cuts.");
...
fastjet::JetAlgorithm AlgoFromString(std::string s)
{
   ...
   if (s.substr(0, 6) == "antikt")
      return fastjet::antikt_algorithm;
   if (s.substr(0, 6) == "cambri")
